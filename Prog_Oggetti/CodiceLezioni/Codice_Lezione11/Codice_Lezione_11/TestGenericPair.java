...
/* Questo metodo, dato un (x,y) di tipo GenericPair<T,S>, restituisce un nuovo (y,x) di tipo GenericPair<S,T>. Attenzione qui sotto a non scrivere GenericPair al posto di GenericPair<S,T> come tipo di ritorno, altrimenti il compilatore e' costretto a indovinare S, T, e rimpiazza GenericPair con GenericPair<Object,Object>, perdendo informazioni. */
 ...
 public static void main(String[] args){
  /* Qui creiamo un GenericPair<T,S> in cui il primo elemento (campo first) è di tipo T=String e il secondo (campo second) è di tipo S=Integer. Al posto di GenericPair<String,Integer> dopo la new posso scrivere GenericPair<>, purche' la versione del compilatore Java sia in grado di dedurre in modo unico i valori di S, T.*/  
  ...
  GenericPair<Integer,String> q = inv(p);
  System.out.println( "p non cambia: \t" + p);
  //Dato che inv e' un metodo statico, al di fuori della sua 
