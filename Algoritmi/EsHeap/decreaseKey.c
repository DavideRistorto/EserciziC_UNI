...
void heapify(Heap H, int i) {
  ...
  if (l != -1 && H->keys[l] < H->keys[smallest]) smallest = l;
  if (r != -1 && H->keys[r] < H->keys[smallest]) smallest = r;
...
void buildMinHeap(Heap H) {
  ...
  for (int i = H->hd / 2 - 1; i >= 0; i--) heapify(H, i);
}
...
void showHeap(Heap H) {
  ...
  for (int i = 0; i < H->hd; i++) printf("%d ", H->keys[i]);
  printf("\n");
}
