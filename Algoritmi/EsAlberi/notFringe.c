...
// post:restituisce una lista con tutti i nodi che non sono foglie
list notFringe(Tree t) {
  list result = NULL;
  // caso base
  if (t == NULL) return NULL;
  // se non è una foglia l'aggiungo alla lista
  if (t->child != NULL) {
    result = Cons(t->info, NULL);
    Tree current = t->child;
    while (current) {
      result = concat(result, notFringe(current));
